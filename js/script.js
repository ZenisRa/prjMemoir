...
function renderAllTasks() {
    ...
    const testoRicerca = document.getElementById('searchInput').value.toLowerCase();
...
function creaHtmlTask(task, container) {
    ...
    if(task.priority === "1") iconaPriorita = "<span class='priority-indicator prio-1'>!</span>";
    if(task.priority === "2") iconaPriorita = "<span class='priority-indicator prio-2'>!!</span>";
    if(task.priority === "3") iconaPriorita = "<span class='priority-indicator prio-3'>!!!</span>";

    const riga = document.createElement('div');
    riga.className = 'task-row';
    // Al click apre i dettagli
    riga.onclick = function() { apriDettagli(task.id); };

    // Mostra un placeholder se il titolo è vuoto (per task non salvati)
    const titoloMostrato = task.title && task.title.trim() !== '' 
        ? pulisciTestoHTML(task.title) 
        : (task.isNew ? '(Nuovo promemoria - non salvato)' : 'Senza titolo');
...
function switchFilter(filtroNome, elemento) {
    ...
    if(elemento) {
        ...
        if(elemento.classList.contains('list-item')) elemento.classList.add('active');
        else elemento.closest('.list-item').classList.add('active');
    } else {
        // Se cambiato via codice (es. all'avvio)
        if(filtroNome === 'today') document.getElementById('filter-today')?.classList.add('active');
        else if(filtroNome === 'tomorrow') document.getElementById('filter-tomorrow')?.classList.add('active');
        else if(filtroNome === 'upcoming') document.getElementById('filter-upcoming')?.classList.add('active');
        else if(filtroNome === 'all') document.getElementById('filter-all')?.classList.add('active');
    }
...
function apriDettagli(idTask) {
    ...
    if (task) {
        ...
        document.getElementById('detList').value = task.list || (elencoListe[0] || "");
    }
}
...
function closeDetails() {
    ...
    if (idPromemoriaCorrente) {
        ...
        if (task && task.isNew) {
            ...
            elencoPromemoria = elencoPromemoria.filter(t => t.id != idPromemoriaCorrente);
            aggiornaInterfaccia();
        }
    }
...
function saveTask() {
    ...
    const indice = elencoPromemoria.findIndex(t => t.id == idPromemoriaCorrente);
    ...
    if (indice > -1) {
        ...
        const datiForm = {
            ...
            action: (taskCorrente.isNew || taskCorrente.id > 2000000000) ? 'create' : 'update',
            id: taskCorrente.id
        };
...
function aggiornaTitoloHeader() {
    ...
    if(filtroCorrente === 'all') titolo = "🗂️ Tutti";
